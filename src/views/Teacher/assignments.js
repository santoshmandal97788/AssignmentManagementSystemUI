...
export default class TeacherAssignments extends Component{
    ...
    getAssignments = () =>{
        Axios.get(Consts.BaseUrl+"TeacherMobile")
        .then(res=>{
            var data = res.data;
            this.setState({
                loading:false,
                        ...
                        this.state.assignments.map((data,id)=>{
                            return(
                                <tr key={id}>
                                    ...
                                    <td>{dateFormat(data.Assignment_Release_Date,"yyyy-mm-dd")}</td>
                                    ...
                                    <td>
                                        <a color="info" className="fa fa-user text-info h4" title="Show Detail" onClick={()=>{this.showDetail(data)}}></a>
                                        {" "}
                                        <a href={"#/show-students?id="+data.Id} title="Show Students" className="fa fa-users text-primary h4"></a>
                                        {" "}
                                        <a className="fa fa-download text-success h4" title="Download" onClick={(id)=>{this.downloadFile(data.Id,data.Name)}}></a>
                                    </td>
                                </tr>
                            )
                        })

...
export default class ListAssignmentByRoutine extends Component{
    constructor(props){
        super(props);
        this.state={
            showDetail:false,
    ...
    getRoutine= ()=>{
        if((this.state.Year_Batch_Id&&this.state.Faculty_Id&&this.state.Semester_Id&&this.state.Section_Id)!=""){
            Axios.get(Consts.BaseUrl+"AssignmentRoutine?year="+this.state.Year_Batch_Id+"&&faculty="+this.state.Faculty_Id+"&&semester="+this.state.Semester_Id+"&&section="+this.state.Section_Id)
        .then(res=>{
            var data = res.data;
            var ddroutine=[];
            for(var i=0; i<data.length; i++){
                var singleRoutine = {}
                singleRoutine.value=data[i].Id;
                singleRoutine.label=data[i].Assignment_Name;
                ddroutine.push(singleRoutine);
            }
            this.setState({
                ddroutine:ddroutine,
            })
    ...
    updateRoutine = (event)=>{
        ...
        this.setState({
            ...
            this.setState({
                Routine_Id:event.value,
            })
        })
        console.log(this.state.Routine_Id);
    }   
    ...
    showTable = () =>{
        ...
        return  <Datatable paginated routineParam routineId={this.state.Routine_Id} showDetail={this.showDetail} url="SubmittedAssignment" route="assignment" tableName="Assignment" columns="Student=StudentName,Assignment=Assignment_Name,Faculty=FacultyName,Teacher=TeacherName,Submitted Date=Submitted_Date,Semester=SemesterName,Year Batch=YearBatch,Section=SectionName,Location=Assignmnet_Location,Checked=Checked_Status,FeedBack=Feedback_Status,Marking,On Time=onTime,Action"/>
        }
    }
                    ...
                    <Row>
                          ...
                          <Col md="6">
                              ...
                              <Select 
                                                ...
                                                value={this.state.selectedRoutine}
                                                onChange={this.updateRoutine}
                                                options={this.state.ddroutine}
                                                />
                              </FormGroup>
                          </Col>
                    </Row>
